...
export async function registerRepositoryRoutes(app: FastifyInstance) {
  ...
  app.post('/upload', async (request, reply) => {
    ...
    const repository = await repositoryUploadService.uploadZip({
      name: getMultipartFieldValue(file.fields.name) ?? file.filename.replace(/\.zip$/i, ''),
  ...
  app.post('/:id/search', async (request): Promise<SearchCodeResponse> => {
    ...
    return repositoryIndexService.searchRepository(params.id, body.query, body.limit)
  })
  ...
  app.post('/:id/ask', async (request): Promise<AskRepositoryResponse> => {
    ...
    return repositoryIndexService.askRepository(params.id, body.question, body.limit)
  })
  ...
  app.post('/:id/ask/stream', async (request, reply) => {
    const params = repositoryParamsSchema.parse(request.params)
    const body = askRepositorySchema.parse(request.body)
    const context = await repositoryIndexService.prepareAskContext(params.id, body.question, body.limit)
    ...
    try {
      for await (const delta of repositoryIndexService.streamAnswer(context.prompt)) {
...
function writeSseEvent(response: NodeJS.WritableStream, event: string, data: unknown) {
