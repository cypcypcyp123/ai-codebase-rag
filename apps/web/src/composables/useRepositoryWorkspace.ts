...
export function useRepositoryWorkspace() {
  ...
  const indexedCount = computed(() =>
    repositories.value.filter((repository) => repository.status === 'indexed').length
  )
  ...
  async function uploadZip(file: File) {
    ...
    try {
      const repository = await uploadRepositoryZip(file, file.name.replace(/\.zip$/i, ''), {
        onProgress: (percent) => {
  ...
  async function runChunksPreview() {
    ...
    try {
      chunkResult.value = await previewRepositoryChunks(selectedRepositoryId.value)
      await refreshRepositories()
    } catch (error) {
      errorMessage.value = formatError(error)
    } finally {
