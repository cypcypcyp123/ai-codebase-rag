...
export function uploadRepositoryZip(
	file: File,
	name?: string,
	...
	return uploadFormWithProgress<CodeRepository>(
...
export function scanRepository(repositoryId: string) {
	return requestJson<ScanRepositoryResponse>(
		`/api/repositories/${repositoryId}/scan`,
		{
			method: "POST",
			body: JSON.stringify({}),
		},
...
export function previewRepositoryChunks(repositoryId: string) {
	return requestJson<ChunkRepositoryResponse>(
		`/api/repositories/${repositoryId}/chunks`,
		{
			method: "POST",
			body: JSON.stringify({}),
		},
...
export function indexRepository(repositoryId: string) {
	return requestJson<IndexRepositoryResponse>(
...
export function searchRepository(
	...
	return requestJson<SearchCodeResponse>(
...
export function askRepository(
	...
	return requestJson<AskRepositoryResponse>(
		`/api/repositories/${repositoryId}/ask`,
		{
			method: "POST",
			body: JSON.stringify(payload),
		},
...
export async function askRepositoryStream(
	...
	const response = await fetch(
		resolveApiUrl(`/api/repositories/${repositoryId}/ask/stream`),
		{
			method: "POST",
			headers: {
				"content-type": "application/json",
...
function uploadFormWithProgress<T>(
	...
	return new Promise((resolve, reject) => {
		...
		xhr.onerror = () =>
			reject(new Error("上传失败，请确认后端服务是否正常运行"));
		xhr.send(body);
	});
}
